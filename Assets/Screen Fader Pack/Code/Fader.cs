...
#region Fluent interface
    ...
    public IFader FadeIn(UnityEngine.UI.Image img, float time = 1)
    {
        AddTask(new FaderTask()
        {
            State = FadeState.Stop,
    ...
    public IFader FadeOut(UnityEngine.UI.Image img, float time = 1)
    {
        AddTask(new FaderTask()
        {
            State = FadeState.Stop,
...
public interface IFader
{
    ...
    IFader FadeIn(UnityEngine.UI.Image img, float time = 1);
    /// <summary>
    /// Fade-out screen 
    /// </summary>
    ...
    IFader FadeOut(UnityEngine.UI.Image img, float time = 1);
    /// <summary>
    /// Pause
    /// </summary>
...
namespace ScreenFaderComponents
{
    ...
    namespace Actions
    {
        ...
        {
            UnityEngine.UI.Image _obj;
            public CanvasImageFadingAction(FadeDirection direction, UnityEngine.UI.Image obj, float time)
            {
                this.direction = direction;
                this.time = time;
                this._obj = obj;
        ...
        public class GameObjectFadingAction : IAction
        {
            ...
            public GameObjectFadingAction(FadeDirection direction, GameObject obj, float time)
            {
                ...
                images = getComponents<UnityEngine.UI.Image>(obj);
                texts = getComponents<UnityEngine.UI.Text>(obj);
            }

            private T[] getComponents<T>(GameObject go) where T : class
            {
                T[] components = go.GetComponentsInChildren<T>();
                var owncomponents = go.GetComponents<T>();

                if (owncomponents != null && owncomponents.Length > 0)
                {
                    Array.Resize<T>(ref components, components.Length + owncomponents.Length);
                    for (int i = 0; i < owncomponents.Length; i++)
                    {
                        components[components.Length - 1 + i] = owncomponents[i];   
                    }
                }
            ...
            protected virtual void Apply(float value)
            {
                ...
                {
                    Color tmp = renderers[i].material.color;
                    tmp.a = value;
                    renderers[i].material.color = tmp;
                }

                for (int i = 0; i < images.Length; i++)
                {
                    Color tmp = images[i].color;
                    tmp.a = value;
                    images[i].color = tmp;
                }
        ...
        public class ChangeColorAction : IParametrizedAction
        {
            public void Execute(params object[] args)
            {
                ...
                if (inst != null && args[0] is Color)
                    inst.color = (Color)args[0];
